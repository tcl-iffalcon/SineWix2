...
// TMDB API — ücretsiz public key (read-only)
...
function toStreams(videos, label) {
  ...
  console.log("[toStreams] ham:", (videos || []).length, "geçerli:", streams.length);
  return streams;
}

// -------------------------------------------------------
// IMDb ID → başlık (TMDB üzerinden)
// -------------------------------------------------------
function getTitleFromTmdb(imdbId, mediaType) {
  // TMDB /find endpoint'i IMDb ID'yi başlığa çevirir
  ...
  return fetch(url, { headers: { "Accept": "application/json" } })
    ...
    .then(function(data) {
      ...
      if (mediaType === "movie") {
        ...
        // tv bulunamadıysa movie_results'a da bak (tip belirsizse)
        if (!results.length) results = data.movie_results || [];
      }
      if (!results.length) throw new Error("TMDB'de bulunamadı: " + imdbId);

      var item = results[0];
      // Film için title, dizi için name
      var title = item.title || item.name || item.original_title || item.original_name || "";
      console.log("[TMDB] " + imdbId + " -> \"" + title + "\"");
      return title;
    });
}
...
// -------------------------------------------------------
// IMDb ID → SineWix SID
// Strateji: TMDB'den başlığı al → SineWix'te ara → ilk eşleşen SID
// -------------------------------------------------------
function resolveSid(imdbId, mediaType) {
  if (imdbCache[imdbId]) {
    console.log("[resolveSid] önbellek:", imdbId, "->", imdbCache[imdbId]);
    return Promise.resolve(imdbCache[imdbId]);
  }

  return getTitleFromTmdb(imdbId, mediaType)
    .then(function(title) {
      return apiGet("/search/" + encodeURIComponent(title) + "/" + API_KEY)
        .then(function(data) {
          var results = data.search || [];
          console.log("[resolveSid] SineWix arama \"" + title + "\": " + results.length + " sonuç");

          if (!results.length) throw new Error("SineWix'te bulunamadı: " + title);

          // Tip eşleşmesine göre filtrele
          var filtered = results.filter(function(r) {
            if (mediaType === "movie") return r.type === "movie";
            return r.type !== "movie"; // series, anime vb.
          });
          var hit = filtered.length ? filtered[0] : results[0];

          var sid = String(hit.id);
          imdbCache[imdbId] = sid;
          console.log("[resolveSid] bulundu:", imdbId, "->", sid, "(\"" + (hit.name || hit.title) + "\")");
          return sid;
        });
    });
}
    ...
    return resolveSid(p.imdbId, apiType)
      .then(function(sid) {
        ...
        return apiGet("/series/show/" + sid + "/" + API_KEY)
          .then(function(d) { return buildSeriesMeta(id, p.imdbId, sid, d); });
      })
      .catch(function(err) {
...
function buildSeriesMeta(id, imdbId, sid, data) {
  ...
  seasons.forEach(function(season) {
    ...
    (season.episodes || []).forEach(function(ep) {
      ...
      // Video ID: imdbId varsa tt formatında, yoksa sinewix formatında
      var vid = imdbId
        ? imdbId + ":" + sNum + ":" + eNum
        : videoId(sid, sNum, eNum);
      videos.push({
        id:        vid,
